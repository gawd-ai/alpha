...
| `demos/` | Narrated, runnable demos (`walkthrough`, `federation`, `distribute`, `bestiary-live`, `dialogue`; the `cluster/` dir is a runbook, not a packaged demo). The registry of what `alpha demo` can launch is [`demos/demos.json`](demos/demos.json). The fastest way to *see* Alpha. Stay at the root, alongside the door they exercise. |
