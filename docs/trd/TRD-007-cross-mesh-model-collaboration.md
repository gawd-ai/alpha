...
an external operator-signed seal. Product acceptance runs only after exact-SHA push CI through the
protected [v0.5 live-acceptance workflow](../../.github/workflows/v05-live-acceptance.yml), then uses
the packaged candidate binary's standalone `dialogue verify-live` path to validate the retained
bundle without a provider, Git, mutable store, running Sanctum, or private key.

The default/`--fixture` dialogue run is a hermetic regression and exact-replay check. It is valuable
CI evidence for mechanism, but it is not evidence that models made the decisions and cannot make
...
BuildCargo                       BuildBeast                  BuildCritter
  ...
  retained calls/receipts/turns/decisions/sources/artifacts/Job evidence
      ...
      offline exact-binary verifier + encrypted/raw and disclosure-safe packs
                              |
       workflow provenance attestations + immutable external retention
```
...
- **R1 — Product acceptance uses live models.** The acceptance run MUST use `--live` and three
  ...
  in the protected v0.5 live-acceptance workflow after successful push CI for the exact candidate;
  an ad hoc local invocation is not the product gate.
- **R2 — The run is fresh and bound to one exact source identity.** The challenge MUST contain a
  ...
  checks, the external seal, and workflow artifact attestations provide complementary provenance,
...
- **R10 — Evidence retention and operator attestation fail closed.** The evidence directory MUST be
  ...
  exclude prompts/completions, credentials, and private keys. Secrets MUST never enter the evidence
  schema. The workflow's Actions artifacts are temporary staging only. Before tagging, the release
  operator ceremony MUST move them, with digest verification, into an access-controlled immutable
  store retained for the supported release lifetime and retain the external promotion receipt.
...
- **R13 — The release ceremony is protected and externally anchored.** The candidate SHA MUST equal
  the workflow-dispatch SHA, checked-out `HEAD`, embedded build commit, successful push-CI SHA, and
  final retained source identity. The exact dialogue binary MUST build and be copied/hash-pinned
  before protected provider/operator secrets are referenced or materialized. The workflow MUST run
  the same packaged bytes for generation and offline verification, request provenance attestations
  for the binary and both staged packages. Before tagging, the release operator ceremony MUST verify
  immutable promotion and record the verified output in an external signed/append-only acceptance
  and semantic registry. The registry record MUST stay external until the proven commit is tagged,
  so accepting the proof cannot mutate the commit it identifies. Artifact attestations
  MUST NOT be represented as proof of provider weights or reproducible compilation.
...
| Protected ceremony | Exact push CI precedes a reviewer-protected exact-SHA workflow; the binary builds before secrets are materialized and encrypted-raw/disclosure-safe staging packs are separately attested. Before tag, the operator verifies their promotion to immutable supported-lifetime storage and appends the external signed acceptance/semantic record. |
...
The default scripted `dialogue` demo and CI suites are regression evidence only. They do not satisfy
the live rows above. TRD-007 remains **Accepted, not Met**, ADR-0049 remains **Accepted, not
Implemented**, in the frozen v0.5.0 source candidate. Before that exact commit may be tagged, all of
the following must refer to it:

1. constrained push CI is Green for the exact candidate SHA;
2. the protected `v05-live-acceptance.yml` workflow runs a fresh `--live` proof with the complete
   external prior-semantic registry;
3. the same packaged binary's `dialogue verify-live` command accepts the complete evidence under the
   independently pinned commit and seal signer;
4. the encrypted raw package, disclosure-safe pack, exact binary, workflow attestations, and run
   metadata are moved from 90-day Actions staging to immutable supported-lifetime storage; and
5. an external signed/append-only acceptance record names the commit/run, evidence-index digest,
   binary/package digests, authorized seal signer, provider/model labels, semantic digest,
   attestation subjects, retention locations, and result.
