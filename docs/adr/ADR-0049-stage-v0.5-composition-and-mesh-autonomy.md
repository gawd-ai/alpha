...
1. **v0.5.0 acceptance requires one fresh retained live run.** The normative bar is TRD-007. A clean
   exact release commit must first pass the exhaustive local `tools/local-validation.sh` gate, which
   produces a validation report plus copied-binary handoff. The unchanged commit must then pass the
   short hosted sanity gate before the local `tools/v05-live-acceptance.sh` ceremony consumes that
   handoff with distinct Builder, Reviewer, and Contract Tester `mind::Model` injections. That
   ceremony must retain its complete evidence bundle and create an external operator-signed seal over
   the evidence-index root. Scripted/default, fake, replayed, and direct `dialogue --live`
   invocations remain useful regressions or exploratory evidence but are never product acceptance.
   Hosted CI is required merge/tag hygiene but cannot replace either local gate.
...
5. **The evidence distinguishes facts from claims.** Live acceptance retains seven calls: five from
   ...
   remains release-operator policy. The exact commit/binary identities, package digests, seal, offline
   verifier report, and external signed acceptance record complement those proofs but attest neither
   provider weights nor reproducible compilation.
6. **Evidence is durable and exact-commit-bound.** The newly created private evidence directory lives
   ...
   excludes prompts/completions and secrets while retaining the validation report, exact binary,
   signed seal/index, acceptance manifest, six-field verifier report, README, and hashes. Both
   packages, the exact binary, and ceremony metadata move directly to immutable supported-lifetime storage before tag.
   Provider/operator keys and raw evidence never enter GitHub.
...
15. **Acceptance state remains external until tag.** An append-only operator registry supplies the
    ...
    commit/validation/index/binary/package/seal and immutable-retention identities. It remains
...
- The exhaustive local validation tool proves the deterministic mechanisms and negative gates
  without credentials; hosted CI provides only a short sanity signal and cannot make TRD-007 Met.
  The unchanged exact commit passes hosted sanity after local validation and before the separate
  local live-evidence ceremony, avoiding provider calls for a candidate that fails basic hygiene.
...
- The frozen v0.5.0 source candidate keeps TRD-007 Accepted/not Met and this ADR Accepted/not
  ...
  before exact-commit local validation and the retained, operator-sealed live run; post-proof metadata
  edits are a different, unproved commit. The encrypted/safe bundles, exact binary, and provenance
  records must move directly to the release-lifetime immutable store before tag. A later post-tag
  documentation commit links that record and advances status.
...
- **Test posture:** default/`--fixture` plus focused tests cover deterministic regression, exact
  ...
  bounds/tamper checks, and OpenAI-feature compilation. `tools/local-validation.sh` runs the
  authoritative exhaustive credential-free gate once on the frozen commit and prepares its handoff.
  After the unchanged commit passes hosted sanity, product acceptance uses
  `tools/v05-live-acceptance.sh` for the fresh exact-commit live run, offline verifier,
  encrypted/private and disclosure-safe packages, and immutable external retention.
