...
- `env.text` — the payload decoded as **lossy UTF-8** (the engine has no Blob→String builtin, so this
  is how a critter does string work).
...
3. **No in-script JSON.** There's no JSON parser in the sandbox — use `split` + a map (`kv-extract`
