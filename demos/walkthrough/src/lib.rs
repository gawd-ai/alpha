...
//! `sanctum/tests/` already prove (`m3_authoring_loop.rs`, `abode_migrate_local.rs`). It exists for
//! one reason: the most striking properties of the substrate — *an AI authors, compiles, signs, and
//! runs a creature with zero human edits*, and *a running self migrates between Sanctums with
//! cryptographic continuity* — were only ever observable inside `#[test]` functions. Here they run
//! against a real, live kernel and narrate each gate as it passes.
//!
...
//!
//! Everything here uses only the public crate APIs an AI operator would drive — there is no demo-only
...
// =================================================================================================
// Phase 1 — Self-authoring: an AI writes, compiles, signs, and runs a creature. Zero human edits.
// =================================================================================================

fn phase_self_authoring() -> Result<(), String> {
    banner("Phase 1 · Self-authoring — the AI writes, compiles, signs, and runs code itself");
    ...
    ok(&format!(
        "the agent authored crate `{}` ({} bytes of Rust) — no human edits",
        resp.crate_name,
        resp.source.len()
    ));
    ...
    step("sending “abc” to the creature the AI just wrote…");
    let env = request_reply(
        &probe_bus,
        &probe_rx,
    ...
    ok(&format!(
        "got back “{got}”  — the AI authored, compiled, signed, and ran that code itself."
    ));
...
fn phase_critter() -> Result<(), String> {
    ...
    ok(&format!(
        "the agent authored `{}` ({} bytes of Rhai) — no human edits",
        resp.crate_name,
        resp.source.len()
    ));
    ...
    // 4) Run it.
    step("sending “abc” to the critter the AI just wrote…");
    let env = request_reply(
        &probe_bus,
        &probe_rx,
        Dispatch::to(Address::Creature(id), b"abc".to_vec()).with_reply_to(reply_to).with_corr(3),
...
// =================================================================================================
// Phase 2 — Migration: a running self moves between Sanctums, with signed, verified continuity.
// =================================================================================================

fn phase_migration() -> Result<(), String> {
    banner("Phase 2 · Migration — a running self moves between Sanctums, cryptographically intact");
    note("(in-process here for a repeatable demo; abode_migrate_cross_node.rs proves the same over two real TCP sockets)");
    ...
    ok("two Sanctum bodies (A, B) booted for one Abode — single-active-fork semantics");

    let (probe_id, probe_bus, probe_rx) = kernel.open_endpoint(Capabilities::default());
    let reply_to = Address::Creature(probe_id);

    // Seed the self's state on body A.
    step("seeding the self's state on Sanctum A: counter=42");
    let env = request_reply(
        &probe_bus,
        &probe_rx,
        Dispatch::to(
    ...
    match MigratorMsg::parse(&env.payload) {
        Ok(MigratorMsg::StateSet) => ok("Sanctum A is authoritative, holding the self's state"),
    ...
    match MigratorMsg::parse(&env.payload) {
        Ok(MigratorMsg::MigrateAck { migrated_to }) => {
            ok(&format!("B admitted the self; A acknowledges hand-off to {}", migrated_to.0))
        }
    ...
    if let MigratorMsg::StatusReply { state, payload_len, .. } = status(src_id, 3)? {
        ok(&format!(
            "Sanctum A → state=\"{state}\", payload_len={payload_len}  (the self has left A)"
        ));
    }
    if let MigratorMsg::StatusReply { state, payload_len, .. } = status(dest_id, 4)? {
        ok(&format!("Sanctum B → state=\"{state}\", payload_len={payload_len}  (the self is now authoritative on B)"));
    }
...
pub fn run() {
    ...
    println!("An AI authors and ships running code; a running self moves between machines. Watch:");
    ...
    println!(
        "path the test suite uses. The cross-MACHINE version (two real TCP sockets) is proven in"
    );
    println!("sanctum/tests/abode_migrate_cross_node.rs and distributor_cross_node.rs.\n");
}
