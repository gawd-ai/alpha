...
| [**dialogue**](dialogue/) | `cargo run -p dialogue` | **Two agents conversing across a Realm boundary.** Two Sanctums in two Realms over real ed25519-authenticated TCP (loopback): a `dialogue-initiator` on one Realm holds a **multi-turn conversation** with a **stateful agent** on the other, each turn a SEER `dialogue` Query routed across the boundary by the **Omega gateway** — application traffic crossing the mesh, not just catalogue federation. The in-process dress rehearsal for the v0.5.0 "AIs interacting across the mesh" headline. | `dialogue_seam.rs`, `omega_app_routing_cross_node.rs`, `distributor_cross_realm.rs` |
...
- **dialogue** is hermetic (no model, no network beyond loopback): it boots two real Sanctums in two
  Realms and runs a live agent-to-agent conversation across the boundary. Where `federation` moves
